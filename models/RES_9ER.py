...
class ResNet50(tf.keras.Model):
  ...
  def __init__(self, data_format, name='', trainable=True, include_top=True, pooling=None, classes=1):    # modified
    ...
    # self.avg_pool = layers.AveragePooling2D((11, 17), strides=(11, 17), data_format=data_format)  # modified for 10th
  ...
  def call(self, inputs, training=True, mask=None):
    ...
    if self.include_top:
      # x = self.regressor(self.flatten(x))
      # return tf.nn.elu(x)
      return self.regressor(self.flatten(x))
    elif self.global_pooling:
      x = self.global_pooling(x)
      return tf.nn.elu(x)  # Suppress negative predictions
    else:
      return x
