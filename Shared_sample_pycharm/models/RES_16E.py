...
class Res16E(tf.keras.Model):
  ...
  def __init__(self, data_format, name='', trainable=True, include_top=True, pooling=None, classes=1):
    super(Res16E, self).__init__(name=name)
    ...
    self.avg_pool = layers.AveragePooling2D((12, 18), strides=(12, 18), data_format=data_format)  # modified for 5th

    if self.include_top:
      self.flatten = layers.Flatten()
      self.regressor = layers.Dense(classes, activation="elu", name='elu_FC')
